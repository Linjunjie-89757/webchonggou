...
| Detail comment duplication | Goal 51 observed `GET /bugs/{id}` already returns `comments`, and the drawer also requests `/bugs/{id}/comments`. | The extra request is intentional for independent comment error handling, but it may be redundant for small details. | Keep unless network volume becomes a concern; consider reusing detail comments later. |
