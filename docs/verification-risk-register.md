...
| Comment create path | Goal 141 confirmed the backend has `POST /bugs/{id}/comments` with `CreateBugCommentRequest.content`, and the old frontend calls `platformApi.addBugComment`. The rebuilt frontend currently only has `GET /bugs/{id}/comments` and read-only comment rendering. | Users cannot add comments from the rebuilt frontend yet even though the backend contract exists. | Add a minimal comment-create feature after confirming desired placement and disposable comment cleanup expectations. |
| Attachment actions | Goal 142 confirmed the backend has `POST /bugs/{id}/attachments`, `GET /bugs/{id}/attachments/{attachmentId}/download`, and `DELETE /bugs/{id}/attachments/{attachmentId}`; the old frontend supports upload/download/delete. The rebuilt frontend currently only displays attachments returned by detail data. | Upload, download, and delete workflows are unavailable from the rebuilt frontend. | Add attachment actions with disposable files and permission checks when file workflow enters scope. |
...
| Attachment real-data coverage | Goal 51 saw an empty `attachments` list, and Goal 143 only rechecked the read-only attachment area with current disposable data. | Attachment rendering with long names, file sizes, upload metadata, images, and authorized download URLs is still unverified. | Test with a disposable defect that has real attachments once upload/download actions are in scope. |
...
| Mutation success paths | Goals 138-140 verified disposable create, edit, assign, and one transition path against the real backend. | Delete remains unavailable; comment create and attachment mutations are contract-confirmed but not implemented in the rebuilt frontend. | Continue with comment and attachment minimal wiring only after scope approval. |
